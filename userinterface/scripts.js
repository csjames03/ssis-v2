...
document.getElementById('addForm').addEventListener('submit', async function (e){
    ...
    const isUnique = await eel.IsUniqueId(code)()
...
async function getResults(searchValue){
    ...
    if(results.length > 0){
        for(let i = 0; i < results.length; i++){
            ...
            div.innerHTML =`
                <p class="text-base font-semibold col-auto">${results[i][0]}</p>
...
document.getElementById('editForm').addEventListener('submit',(e)=>{
    ...
    updateCourse([codeInput,courseInput])
})
...
async function updateCourse(courseInfo){
    ...
    console.log(courses)
    const isUpdated = await eel.UpdateCourse(courseInfo[0],courseInfo[1])()
    if(isUpdated){
        const UpdateStudentCourse = await eel.CourseSearchStudent(courses[0])()
        Swal.fire({
            position: 'top-end',
            icon: 'success',
          ...
          code.innerHTML = courseInfo[0]
          course.innerHTML = courseInfo[1]
    }else{
    }
}
...
async function DisplayCourses(course){
    ...
    div.innerHTML =`
        <p class="text-medium font-semibold text-black truncate " id="${course[0]}-course">${course[1]}</p>
        <p class="text-sm font-bold truncate" id="${course[0]}-code">${course[0]}</p>
...
async function deleteCourse(){
    ...
    const numOfStudentEnrolled = await eel.CourseSearchStudent(courseInfo[0])()
